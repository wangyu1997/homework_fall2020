...
class RL_Trainer(object):
    ...
    def run_training_loop(self, n_iter, collect_policy, eval_policy,
                          initial_expert_data=None, relabel_with_expert=False,
        ...
        for itr in range(n_iter):
            ...
            paths, env_steps_this_batch, train_video_paths = training_returns
    ...
    def collect_training_trajectories(
            self,
            itr,
            load_initial_expert_data,
            collect_policy,
            batch_size,
    ):
        """
        :param itr:
        :param load_initial_expert_data:  path to expert data pkl file
        ...
        if itr == 0:
            import pickle
            data_file = open(load_initial_expert_data, 'rb', encoding='utf-8')
            expert_data = pickle.load(data_file)
        ...
        paths, env_steps_this_batch = utils.sample_trajectories(self.env, collect_policy, batch_size,
                                                                self.params['ep_len'])
    ...
    def train_agent(self):
        ...
        for train_step in range(self.params['num_agent_train_steps_per_iter']):
            ...
            train_log = self.agent.train(self, ob_batch, ac_batch, re_batch, next_ob_batch, terminal_batch)
            all_logs.append(train_log)
        return all_logs

    @classmethod
    def do_relabel_with_expert(cls, expert_policy, paths):
        ...
        for idx, path in enumerate(paths):
            label = expert_policy.get_action(path['observation'])
            paths[idx]['action'] = label
